...
function toNumberStrict(v) {
  ...
  // quita símbolos/espacios de moneda
  s = s.replace(/\s*(USD|US\$|\$|Bs\.?|VEF|VES)\s*/gi, "");

  const hasComma = s.includes(",");
  const hasDot   = s.includes(".");

  // 1) Si hay COMA y PUNTO -> decide por último separador (tu lógica original)
  if (hasComma && hasDot) {
    const lastComma = s.lastIndexOf(",");
    const lastDot   = s.lastIndexOf(".");
    const decSep    = lastComma > lastDot ? "," : ".";
  ...
  // 2) Solo COMAS: ¿patrón de miles 1,234 o 12,345,678?
  if (hasComma) {
    if (/^\d{1,3}(?:,\d{3})+$/.test(s)) {
      // tratar coma como separador de miles
      const n = parseFloat(s.replace(/,/g, ""));
      return Number.isFinite(n) ? n : 0;
    }
    // si no es patrón de miles, tratar coma como decimal
    const n = parseFloat(s.replace(",", "."));
    return Number.isFinite(n) ? n : 0;
  }

  // 3) Solo PUNTOS: ¿patrón de miles 1.234 o 12.345.678?
  if (hasDot) {
    if (/^\d{1,3}(?:\.\d{3})+$/.test(s)) {
      // tratar punto como separador de miles
      const n = parseFloat(s.replace(/\./g, ""));
      return Number.isFinite(n) ? n : 0;
    }
    // caso normal: punto decimal
    const n = parseFloat(s);
    return Number.isFinite(n) ? n : 0;
  }
